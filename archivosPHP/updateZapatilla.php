...
if(isset($parameters)){
	...
	if(JSONCorrectoAnnadir($mensajeRecibido)){
		...
		$zapatilla = $mensajeRecibido["zapatillaAnnadir"]; 
		
		$nombre = $zapatilla["nombre"];
		$id = $zapatilla["id"];
		$query  = "UPDATE zapatillas set nombre = '$nombre' WHERE id = '$id'";
		
		$query .= "VALUES ($id,'$nombre')";
		...
		if (isset ( $result ) && $result) { // Si pasa por este if, la query está está bien y se ha insertado correctamente
			...
			$arrMensaje["mensaje"] = "Zapatilla insertada correctamente";
